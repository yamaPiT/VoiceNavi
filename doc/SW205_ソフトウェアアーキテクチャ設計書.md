...
- **責任**: デモ経路に沿った自車アイコンの移動を計算・管理する。
  ...
  - **【重要】**LLMの先走り（幻覚）を防ぐため、250m以上先の情報については、全ランドマークを渡すのではなく「進行方向にある最も近い1つのランドマークのみ」を抽出して提供するアーキテクチャとする。
...
### 4.2 音声対話フロー
1. `VoiceInteractionModule` がユーザーの音声を認識 (STT) または `Application` がイベントを検知（目的地到着、または**無言時の自律発話タイマー発火**など）。
