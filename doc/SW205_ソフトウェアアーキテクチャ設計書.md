...
flowchart TD
    ...
    subgraph Frontend ["フロントエンド (Vite)"]
        direction TB
        UI_Ctrl["UIController - ボタン, 状態, 速度表示"]
        Sim["Simulation Engine - 位置計算, 速度管理"]
        Map["MapModule - Google Maps API 連携"]
        Voice["VoiceInteractionModule - STT / 音声再生"]
        LLM["LLMAgent - BFF連携クライアント"]
    ...
    subgraph Backend ["バックエンド - Node.js/Express BFF"]
        direction TB
        Server["Express Server - API Gateway and Proxy"]
    ...
    subgraph External ["外部API群"]
        ...
        Gemini["Google Gemini API - @google/generative-ai SDK"]
    end
